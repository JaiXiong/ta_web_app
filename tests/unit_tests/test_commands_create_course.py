...
class TestCommandsCreateCourse(TestCase):
    ...
    def test_CommandsCreateCourse8(self):
        ...
        self.assertEqual(response, "Course CS-520 has been added to the data base with lab sections 001/002/003")
        count = Course.objects.filter(name="CS-520").count()
        self.assertEqual(count, 1)

    # test that 3 lab sections are created for this course
    def test_CommandsCreateCourse9(self):
        co = Commands()
        co.current_user = self.Account1
        co.create_course("CS-520", "004", "M/W/F", "09:00-10:00", "001/002/003")
